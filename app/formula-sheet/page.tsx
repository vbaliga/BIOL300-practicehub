  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
        {f.chapter !== undefined && (
          <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: "0.07em", color: "rgba(var(--gold-rgb),0.45)", background: "rgba(var(--gold-rgb),0.08)", border: "1px solid rgba(var(--gold-rgb),0.15)", borderRadius: 100, padding: "2px 7px", flexShrink: 0 }}>
            Ch.{f.chapter}
          </span>
        )}
        <span style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.09em", color: "rgba(var(--text-rgb),0.38)" }}>
          {f.name}
        </span>
      </div>

      <div
        style={{ color: hovered ? GOLD_LIGHT : "rgba(var(--text-rgb),0.85)", fontSize: "0.9em", overflowX: "auto", transition: "color 0.2s ease" }}
        dangerouslySetInnerHTML={{ __html: katex.renderToString(f.latex, { throwOnError: false, displayMode: true }) }}
      />

      {!hovered && (
        <div style={{ marginTop: 6, fontSize: 10, color: "rgba(var(--text-rgb),0.15)", letterSpacing: "0.04em" }}>
      ...
      <div style={{ maxHeight: hovered ? "240px" : "0px", opacity: hovered ? 1 : 0, overflow: "hidden", transition: "max-height 0.3s cubic-bezier(0.16,1,0.3,1), opacity 0.25s ease" }}>
        <div style={{ borderTop: "1px solid rgba(var(--gold-rgb),0.16)", marginTop: 12, paddingTop: 12 }}>
          <p style={{ fontSize: 12, color: "rgba(var(--text-rgb),0.7)", lineHeight: 1.65, margin: "0 0 10px" }}>{f.explanation}</p>
          <div style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
            ...
            <p style={{ fontSize: 11, color: "rgba(var(--text-rgb),0.42)", lineHeight: 1.6, margin: 0 }}>{f.when}</p>
          </div>
        </div>
      </div>
    </div>
      ...
      <div style={{ minHeight: "100vh", background: BLUE_DARK }}>
        ...
        <main style={{ maxWidth: 960, margin: "0 auto", padding: "40px 24px 72px" }}>
          <div style={{ marginBottom: 32 }}>
            <p style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.12em", color: "rgba(var(--gold-rgb),0.55)", margin: "0 0 10px" }}>Reference</p>
            <h1 style={{ fontSize: 30, fontWeight: 800, color: "var(--text)", letterSpacing: "-0.025em", margin: "0 0 8px" }}>Formula Sheet</h1>
          ...
          <input
            ...
            style={{ width: "100%", boxSizing: "border-box", marginBottom: 20, padding: "9px 14px", borderRadius: 8, border: "1px solid rgba(var(--gold-rgb),0.22)", background: "rgba(var(--text-rgb),0.04)", color: "var(--text)", fontSize: 13, outline: "none" }}
          />
            ...
            {visibleSections.map(section => (
              <div key={section.id}>
                <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 14 }}>
                  ...
                  <h2 style={{ fontSize: 13, fontWeight: 700, color: "rgba(var(--text-rgb),0.7)", margin: 0 }}>{section.title}</h2>
                  <div style={{ flex: 1, height: 1, background: "rgba(var(--gold-rgb),0.1)" }} />
                  <span style={{ fontSize: 10, color: "rgba(var(--text-rgb),0.2)" }}>{section.formulas.length} formula{section.formulas.length !== 1 ? "s" : ""}</span>
                </div>
          ...
          <div style={{ marginTop: 48, padding: "18px 24px", borderRadius: 12, border: "1px solid rgba(var(--text-rgb),0.06)", background: "rgba(var(--text-rgb),0.02)", textAlign: "center" }}>
            <p style={{ fontSize: 11, color: "rgba(var(--text-rgb),0.25)", margin: 0 }}>
              BIOL 300 · UBC
            </p>
          </div>

          <footer style={{ marginTop: 24, textAlign: "center", fontSize: 11, color: "rgba(var(--text-rgb),0.2)", paddingBottom: 16 }}>
